...
const HistoryPage: React.FC = () => {
    // Dummy data with yantras (each yantra now includes quantity)
    const yantraList = [
    ...
    const historyData = Array.from({ length: 30 }, (_, i) => ({
        ticketNumber: `TCK-${1001 + i}`,
        tickets: Math.floor(Math.random() * 5) + 1,
        points: Math.floor(Math.random() * 100) + 10,
        result: Math.random() > 0.5 ? "Win" : "Lose",
        time: `2025-09-13 ${9 + (i % 12)}:${i % 2 ? "30" : "00"} AM`,
        yantras: Array.from(
            { length: Math.floor(Math.random() * 3) + 1 }, // 1–3 yantras
            () => {
    ...
    return (
        <div className="bg-white/80 backdrop-blur-xl shadow-lg rounded-2xl p-6">
            <h2 className="text-2xl font-bold text-slate-800 mb-4">Bet History</h2>

            {/* Table-like Accordion */}
                ...
                {currentRows.map((row, idx) => {
                    ...
                    return (
                        <div key={idx} className="border-b border-slate-200">
                            ...
                            <div
                                onClick={() =>
                                    setOpenRow(isOpen ? null : rowIndex)
                                }
